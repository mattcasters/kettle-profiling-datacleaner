...
public class ModelerHelper extends AbstractXulEventHandler implements ISpoonMenuController {
  ...
  public static void launchDataCleaner( String confFile, String jobFile, String datastore, String dataFile ) {
    ...
    try {
      ...
      String[] jarFolders = new String[] {
        pluginPath + File.separator + "lib",
        pluginPath + File.separator + ".." + File.separator + "extra"
      };

      for ( String jarFolder : jarFolders ) {

        FileObject[] files = KettleVFS.getFileObject( jarFolder ).findFiles( new FileSelector() {
          ...
          public boolean includeFile( FileSelectInfo info ) throws Exception {
            return info.getFile().getName().getBaseName().matches( ".*\\.jar$" );
          }
        } );
        log.logBasic( "Found " + files.length + " libaries in folder " + jarFolder );
        for ( FileObject file : files ) {
          classPath.append( File.pathSeparator ).append( file.getName().getPath() );
        }
      }
